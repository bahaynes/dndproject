...
async def create_game_session(session: schemas.GameSessionCreate, db: Session = Depends(get_db)):
    result = crud.create_game_session(db=db, session=session)
    await event_service.broadcast("session_update")
    return result
...
async def update_game_session(session_id: int, session_update: schemas.GameSessionCreate, db: Session = Depends(get_db)):
    session = crud.get_game_session(db, session_id=session_id)
    if not session:
        raise HTTPException(status_code=404, detail="Game session not found")
    result = crud.update_game_session(db, session=session, session_update=session_update)
    await event_service.broadcast("session_update")
    return result
...
async def signup_for_session(
    ...
    result = crud.add_character_to_game_session(db, session=session, character=character)
    await event_service.broadcast("session_update")
    return result

@router.delete("/{session_id}/signup", response_model=schemas.GameSession, tags=["Game Sessions"])
async def cancel_signup_for_session(
    session_id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_active_user)
):
    ...
    result = crud.remove_character_from_game_session(db, session=session, character=character)
    await event_service.broadcast("session_update")
    return result
